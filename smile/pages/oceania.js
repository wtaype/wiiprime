...
let ciudadesFiltradas = [...wiCiudades.oceania]; // 🔄 oceania
let limaHora = null;
...
export const render = async () => {
  inicializarLimaHora();
  return `<div class="wicontinente"><div class="wicont_header"><h1 class="wicont_title">🏝️ Oceanía</h1><p class="wicont_subtitle">Explora las zonas horarias de las islas del Pacífico</p><div class="wicont_controls"><div class="wicont_search"><i class="fas fa-search wicont_search_icon"></i><input type="text" id="searchOceania" placeholder="Buscar ciudad o país..." class="wisearch_input" /><span class="wicont_count">${ciudadesFiltradas.length} ciudades</span></div><div class="wibtn_group"><button class="wibtn wibtn_format ${clockFormat === '24' ? 'active' : ''}" data-format="24"><i class="fas fa-clock"></i> 24h</button><button class="wibtn wibtn_format ${clockFormat === '12' ? 'active' : ''}" data-format="12"><i class="far fa-clock"></i> 12h</button></div></div></div><div class="wicont_grid" id="oceaniaGrid">${renderCiudades()}</div><div class="wicont_footer">${renderPaginacion()}</div></div>`;
};
...
export const init = () => {
  ...
  $('#searchOceania').on('input', function() {
    ...
    clockIntervals.forEach(clearInterval);
    clockIntervals = [];
    $('#oceaniaGrid').html(renderCiudades());
  ...
  console.log('🏝️ Módulo de Oceanía inicializado');
};
