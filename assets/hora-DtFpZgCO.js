import{S as _,a as m,c as u,$ as d,i as h,e as g}from"./main-DX3Uq-7_.js";import"./vendor-B2AETYxa.js";let t=[],c=new Map;const p=s=>{if(s.ciudad==="Lima")return'<i class="fas fa-map-marker-alt"></i> Tu Ciudad';const i=h("America/Lima"),a=h(s.zona);if(!i?.gmt||!a?.gmt)return"";const r=n=>parseInt(n.match(/GMT([+-]?\d+)/)?.[1]||0),o=r(a.gmt)-r(i.gmt);return o===0?'<i class="fas fa-exchange-alt"></i> Misma hora':o>0?`<i class="fa-solid fa-rotate-right"></i> +${o}h`:`<i class="fa-solid fa-rotate-left"></i> ${o}h`},z=()=>{let s="";for(let i=0;i<60;i++){const a=i*6,r=i%5===0?"wihora_mark wihora_mark_hour":"wihora_mark wihora_mark_minute";s+=`<div class="${r}" style="transform: rotate(${a}deg) translateY(-90px)"></div>`}return s},$=(s,i,a,r)=>{const o=s.data("zona");c.has(o)&&cancelAnimationFrame(c.get(o));const e=requestAnimationFrame(()=>{const l=r*6,v=a*6+r*.1,w=i%12*30+a*.5;s.find(".wihora_hand_second").css("transform",`translateX(-50%) rotate(${l}deg)`),s.find(".wihora_hand_minute").css("transform",`translateX(-50%) rotate(${v}deg)`),s.find(".wihora_hand_hour").css("transform",`translateX(-50%) rotate(${w}deg)`)});c.set(o,e)},f=(s,i)=>{try{const a=h(s.zona);if(!a)return;s.ciudad==="Lima"&&d(".wihora_title").text(`${_()} Gorila`);const[r,o,n]=a.hora.split(":").map(Number),e=g(a.hora),l=p(s);i.removeClass("es-dia es-noche").addClass(e?"es-dia":"es-noche"),$(i,r,o,n),i.find(".wihora_time").text(a.hora),i.find(".wihora_fecha").text(a.fecha),i.find(".wihora_gmt").text(a.gmt),i.find(".wihora_estacion").text(a.estacion),i.find(".wihora_diferencia").html(l),i.find(".wihora_icon i").attr("class",`fas ${e?"fa-sun":"fa-moon"}`)}catch(a){console.error(`❌ Error en ${s.ciudad}:`,a.message)}},M=async()=>{const s=m.principales,i=z();return`
    <div class="wihora">
