import{i as v,S as w,a as p,c as $,$ as c,s as g,g as b,e as z}from"./main-3W8HllkB.js";import"./vendor-B2AETYxa.js";let h=[],e=b("wiClockFormat")||"24",u=null;const x=s=>{if(!u)return"";const[i,a]=u.split(":").map(Number),[o,t]=s.split(":").map(Number),n=i*60+a;let r=o*60+t-n;r>720&&(r-=1440),r<-720&&(r+=1440);const m=Math.floor(Math.abs(r)/60),d=Math.abs(r)%60;return r===0&&d===0?'<span class="misma-hora">Misma hora</span>':r>0?`<span class="adelantado">Adelantado ${d>0?`+${m}h ${d}m`:`+${m}h`}</span>`:`<span class="atrasado">Atrasado ${d>0?`-${m}h ${d}m`:`-${m}h`}</span>`},A=s=>{const[i,a,o]=s.split(":").map(Number),t=i>=12?"PM":"AM";return`${(i%12||12).toString().padStart(2,"0")}:${a.toString().padStart(2,"0")}:${o.toString().padStart(2,"0")} ${t}`},f=(s,i)=>{try{const a=v(s.zona);if(!a)return;if(s.ciudad==="Lima"){u=a.hora;const l=w(a.hora);c(".wihora_title").html(`${l} Gorila`)}const o=e==="12"?A(a.hora):a.hora,t=z(a.hora),n=s.ciudad!=="Lima"?x(a.hora):"";i.find(".wihora_time").text(o),i.find(".wihora_fecha").text(a.fecha),i.find(".wihora_gmt").text(a.gmt),i.find(".wihora_estacion").text(a.estacion),i.find(".wihora_diferencia").html(n),i.find(".wihora_icon i").attr("class",`fas ${t?"fa-sun":"fa-moon"}`)}catch(a){console.error("Error actualizando reloj:",a)}},M=async()=>{const s=p.principales,i=v("America/Lima");return`
    <div class="wihora">
