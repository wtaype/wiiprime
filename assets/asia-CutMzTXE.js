import{a as b,$ as a,b as k,s as z,g as M,i as $,c as C,e as I}from"./main-IrRlgTHq.js";import"./vendor-B2AETYxa.js";let e=[],m=M("wiClockFormat")||"24",l=1;const _=9;let f=[...b.asia],v=null;const E=o=>{const[i,t,s]=o.split(":").map(Number),c=i>=12?"PM":"AM";return`${(i%12||12).toString().padStart(2,"0")}:${t.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")} ${c}`},P=o=>{if(!v)return"";const[i,t]=v.split(":").map(Number),[s,c]=o.split(":").map(Number),n=i*60+t;let r=s*60+c-n;r>720&&(r-=1440),r<-720&&(r+=1440);const h=Math.floor(Math.abs(r)/60),u=Math.abs(r)%60;return r===0&&u===0?'<span class="wicont_diff misma-hora">Misma hora</span>':r>0?`<span class="wicont_diff adelantado">Adelantado ${u>0?`+${h}h ${u}m`:`+${h}h`}</span>`:`<span class="wicont_diff atrasado">Atrasado ${u>0?`-${h}h ${u}m`:`-${h}h`}</span>`},g=(o,i)=>{try{const t=$(o.zona);if(!t)return;o.ciudad==="Lima"&&(v=t.hora);const s=m==="12"?E(t.hora):t.hora,c=I(t.hora),n=P(t.hora),d=c?"Día":"Noche";i.find(".wicont_time").text(s),i.find(".wicont_fecha").text(t.fecha),i.find(".wicont_gmt").text(t.gmt),i.find(".wicont_estado").text(d),i.find(".wicont_diferencia").html(n),i.find(".wicont_icon i").attr("class",`fas ${c?"fa-sun":"fa-moon"}`)}catch(t){console.error("Error actualizando reloj:",t)}},x=()=>{const o=$("America/Lima");o&&(v=o.hora)},w=()=>{const o=(l-1)*_,i=o+_,t=f.slice(o,i);return t.length===0?`<div class="wicont_empty">
